...
when `UserNotificationChannel` has multiple dependencies. You _could_ write
...
Some notes on how it works:
 ...
 - The parameters of the factory method or callback will be mapped to the target
   service's parameters _by name_ during compilation. This means that the
   factory's parameters' positions and order can be arbitrary - they don't have
   to be in the same order as the target service's parameters.
