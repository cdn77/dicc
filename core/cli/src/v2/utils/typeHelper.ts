...
import { ForeignServiceReflection } from '../container';
import {
  AccessorType,
  Argument,
...
export class TypeHelper {
  ...
  resolveCallable(type: Type, ctx: UserCodeContext): Callable {
  ...
  resolveCallableProperty(
    ...
    return node && this.resolveCallable(node.getType(), { ...ctx, node });
  }
  ...
  resolveArguments(signature: Signature, ctx?: Node): Map<string, Argument> {
    const args: Map<string, Argument> = new Map();
    ...
    for (const arg of signature.getParameters()) {
      ...
      args.set(arg.getName(), new Argument(
        type,
  ...
  resolveValueType(rawType: Type, node: Node): ValueType {
    ...
    if (this.refs.isType(type, 'ScopedRunner')) {
      ...
      return new IterableType(rawType, getFirst(type.getTypeArguments()), nullable);
    } else if (this.isAsyncIterable(rawType)) {
      return new IterableType(rawType, getFirst(type.getTypeArguments()), nullable, true);
    } else if (this.isPromise(type)) {
      return this.resolvePromiseType(rawType);
    } else {
      return this.resolveListType(rawType)
        ?? this.resolveCallableType(rawType, node)
        ?? this.resolveSingleType(rawType);
    }
  }

  private resolveCallableType(type: Type, node: Node): ValueType | undefined {
    ...
    if (p1) {
      return returnType.isVoid() ? new InjectorType(type, p1.getTypeAtLocation(node)) : undefined;
    } else if (this.isPromise(returnType)) {
      return new AccessorType(type, this.resolvePromiseType(returnType));
    } else {
      return new AccessorType(type, this.resolveListType(returnType) ?? this.resolveSingleType(type));
    }
  }
  ...
  private resolveListType(rawType: Type, nullable_?: boolean): ListType | undefined {
    ...
    return type.isArray() || type.isReadonlyArray()
      ? new ListType(rawType, type.getArrayElementType()!, nullable)
      : undefined;
  }

  private resolveSingleType(rawType: Type, nullable?: boolean): SingleType {
    return new SingleType(rawType, ...this.unwrapNullable(rawType, nullable));
  }
  ...
  resolveReturnType(signature: Signature): [Type, ReturnType | undefined] {
    ...
    if (this.isIterable(type)) {
      return [raw, new IterableType(raw, getFirst(type.getTypeArguments()), nullable)];
    } else if (this.isAsyncIterable(raw)) {
      return [raw, new IterableType(raw, getFirst(type.getTypeArguments()), nullable, true)];
    } else if (this.isPromise(type)) {
  ...
  resolveAutoImplementedMethod(
    ...
    return new AutoImplementedMethod(name, args, rawReturnType, returnType, method);
  }
  ...
  * resolveExternalContainerServices(
    container: Type,
    map: 'public' | 'dynamic',
  ): Iterable<ForeignServiceReflection> {
  ...
  private * resolveExternalContainerServiceMap(
    ...
    for (const prop of map.getProperties()) {
      ...
      yield { id: prop.getName(), type, aliases, async };
    }
  }
}
